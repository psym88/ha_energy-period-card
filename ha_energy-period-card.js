...
const CARD_VERSION = "1.1.0-beta.5";
...
class HaEnergyPeriodCard extends HTMLElement {
  ...
  _resetToCurrentPeriod() {
    return this._setPeriod(this._active || "today");
  }
  ...
  _render() {
    ...
    this.shadowRoot.innerHTML = `
      <style>
        ...
        div.wrapper .date-navigation {
          grid-template-columns:
            calc(30px + 2 * var(--ha-space-3, 12px))
            max-content
            minmax(0, 1fr)
            calc(30px + 2 * var(--ha-space-3, 12px));
          min-height: calc(30px + 2 * var(--ha-space-3, 12px));
          border-radius: var(--ha-card-border-radius, 12px);
          background: var(--card-background-color);
        }
        div.wrapper .date-navigation button[data-shift] {
          width: calc(30px + 2 * var(--ha-space-3, 12px));
        }
        .title {
        ...
        .date-navigation {
          display: grid;
          grid-template-columns:
            30px max-content minmax(0, 1fr) 30px;
          min-height: 30px;
          overflow: hidden;
        ...
        .date-navigation select {
          ...
          border-radius: 0;
          padding:
            0 var(--ha-space-6, 24px)
            0 var(--ha-space-3, 12px);
    ...
    this.shadowRoot
      ...
      ?.addEventListener("click", () => this._resetToCurrentPeriod());
    this._rendered = true;
    this._updateControl();
  }
}
